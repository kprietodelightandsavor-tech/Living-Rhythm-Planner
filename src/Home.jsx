...
// ── OUTDOOR WEEKLY CIRCLE ─────────────────────────────────────────────────────
function OutdoorWeeklyCircle({ outdoorMins }) {
  const GOAL_MINS = 15 * 60 // 15 hours in minutes
  const weekKey = (() => {
  ...
  // Load all days this week and sum minutes
  const weekMins = (() => {
    let total = outdoorMins // today is already tracked
    try {
      Object.keys(localStorage).forEach(k => {
        if (k.startsWith('lrp-outdoor-mins-') && k !== `lrp-outdoor-mins-${todayStr()}`) {
          ...
          const weekStart = new Date(); weekStart.setDate(weekStart.getDate() - (weekStart.getDay() === 0 ? 6 : weekStart.getDay() - 1)); weekStart.setHours(0,0,0,0)
          if (d >= weekStart) total += val
        }
      })
    } catch {}
    return total
    ...
    <div style={{ background: 'var(--nature-bg)', border: '1px solid var(--nature)', borderRadius: 'var(--r)', padding: '0.85rem 1rem', marginBottom: '0.85rem', display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
      ...
      <div style={{ position: 'relative', width: 110, height: 110, flexShrink: 0 }}>
        <svg width="110" height="110" viewBox="0 0 110 110" style={{ transform: 'rotate(-90deg)' }}>
          ...
          <circle cx="55" cy="55" r={r} fill="none"
            stroke={reached ? 'var(--gold)' : 'var(--nature)'}
            strokeWidth="9"
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', color: 'var(--navy)', marginBottom: '0.25rem' }}>
          {reached ? 'Weekly goal reached ✦' : `${(goalHrs - hrs).toFixed(1)}h to go`}
        </div>
...
// ── MOTHER CULTURE CARD ───────────────────────────────────────────────────────
// 30 prompts — one per day of the month, cycles automatically
